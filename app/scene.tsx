...
interface Props {atlas:Atlas;lang:Lang;state:SceneState;onSelect:(id:string)=>void;onProgress:(n:number)=>void;onError:(s:string)=>void}
export default function AnatomyScene({atlas,lang,state,onSelect,onProgress,onError}:Props){
 const host=useRef<HTMLDivElement>(null),latest=useRef(state),select=useRef(onSelect);
 const langRef=useRef(lang),reportError=useRef(onError);
 latest.current=state;select.current=onSelect;langRef.current=lang;reportError.current=onError;
 useEffect(()=>{
  const el=host.current!;let disposed=false,frame=0,dirty=true,ready=false,lastView='',lastReset=-1,lastIsolate='',layoutKey='',amount=0;
  let lastState:SceneState|null=null;
  const abort=new AbortController();
  let renderer:T.WebGLRenderer;
  try{renderer=new T.WebGLRenderer({antialias:true,alpha:false,powerPreference:'high-performance'});}catch{reportError.current(translate(langRef.current,'errorWebgl'));return;}
  ...
  renderer.domElement.setAttribute('aria-label',translate(langRef.current,'sceneAria'));
  ...
  const loadChunk=async(ci:number)=>{
   ...
   groups.forEach((gs,system)=>{const geometry=mergeGeometries(gs,false);if(!geometry)throw new Error(translate(langRef.current,'errorAssemble'));geometries.push(geometry);const mesh=new T.Mesh(geometry,mats.get(system as never));mesh.frustumCulled=false;scene.add(mesh);});
   lastState=null;loaded++;onProgress(Math.round(loaded/atlas.chunks.length*100));dirty=true;
  };
  (async()=>{try{let cursor=0;await Promise.all(Array.from({length:3},async()=>{while(cursor<atlas.chunks.length){const i=cursor++;await loadChunk(i);}}));if(!disposed){ready=true;dirty=true;}}catch(e){if(!disposed)reportError.current(e instanceof Error?e.message:translate(langRef.current,'errorLoadModel'));}})();
  ...
  const contextLost=(e:Event)=>{e.preventDefault();reportError.current(translate(langRef.current,'errorContextLost'));};renderer.domElement.addEventListener('webglcontextlost',contextLost);
 ...
 useEffect(()=>{host.current?.querySelector('canvas')?.setAttribute('aria-label',translate(lang,'sceneAria'));},[lang]);
